  ...
  if (!tr_torrentIsSeed (s->tor))
    {
      ...
      pieceListSort (s, PIECES_SORTED_BY_INDEX); /* we need sort pieces by index in live program */
                           ...
                           bool                   get_intervals)
{
  ...
  /*if (s->pieceSortState != PIECES_SORTED_BY_WEIGHT)
    pieceListSort (s, PIECES_SORTED_BY_WEIGHT);*/
  /* we need get pieces by index in live program */
  if (s->pieceSortState != PIECES_SORTED_BY_INDEX)
    pieceListSort (s, PIECES_SORTED_BY_INDEX);
...
void 
tr_annoncePeerPieceHave(tr_session *session, tr_piece_index_t pieceIndex)
{
    int i = 0;
    tr_torrent *tor = session->torrentList;
    ...
    for (i = 0; i < n; ++i)
    {
        ...
        /* notify the peer that we now have this piece */
        tr_peerMsgsHave (PEER_MSGS(peer), pieceIndex);
