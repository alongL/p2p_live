...
tr_sessionInitImpl (void * vdata)
{
  ...
  **/
  ...
  {
      session->tr_concrete_tinyproxy_node[i].tcp_status = TR_EMPTY;
      session->tr_concrete_tinyproxy_node[i].tcp_tinyproxy_socket = 0;
      session->tr_concrete_tinyproxy_node[i].tcp_tinyproxy_event = NULL;
  }

  tr_tcp_tinyproxy_Init(session); //phicomm
...
struct sessionLoadLiveProgramData
{
  tr_session * session;
  tr_ctor * ctor;
  int * setmeCount;
...
static void
sessionLoadLiveProgram (void * vdata)
{
  int i;
  int n = 0;
  struct stat sb;
...
#if 0   /* live program don't from torrent file, it from tinyproxy */
  if (!stat (dirname, &sb)
      && S_ISDIR (sb.st_mode)
      && ((odir = opendir (dirname))))
    {
  ...
  tr_torrent * tor;
  if (tor = tr_liveProgramNew(data->ctor, NULL, NULL))
  {
    tr_list_prepend(&list, tor);
    ++n;
  }
...
tr_torrent **
tr_sessionLoadLiveProgram (tr_session * session,
                        tr_ctor    * ctor,
                        int        fd,
                        int        * setmeCount)
{
  //struct sessionLoadTorrentsData data;
  struct sessionLoadLiveProgramData data;
  ...
  tr_runInEventThread (session, sessionLoadLiveProgram, &data);
  while (!data.done)
    tr_wait_msec (100);
