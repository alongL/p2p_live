 ...
 *
 * $Id: inout.c 14241 2014-01-21 03:10:30Z jordan $
 */
                  ...
                  size_t             buflen)
{
  ...
  tr_piece_index_t localPieceIndex = 0;
  char *shmBaseAddr = NULL;
  int offset = 0;
  tr_shmInfo *shmInfoNode = NULL;
  //tr_piece_index_t lastLocalPieceIndex;


  switch(ioMode)
    {
        case TR_IO_READ:
            /* find local piece index */
            shmInfoNode = tor->shmInfoList;
            while (shmInfoNode)
            {
                if(pieceIndex == shmInfoNode->pieceIndex)
                    break;
                else
                    shmInfoNode = shmInfoNode->shmNext;
            }


            localPieceIndex = shmInfoNode->localPieceIndex;
            offset = tr_pieceOffset (tor, localPieceIndex, pieceOffset, 0);

            shmBaseAddr = tr_getShmBaseAddr();

            memcpy(buf, shmBaseAddr + offset, buflen);
            break;

        case TR_IO_WRITE:
            localPieceIndex = (tor->lastLocalPieceIndex + 1) % MAX_SHM_SIZE;
            ...
            tr_shmInsertNode(tor, pieceIndex, localPieceIndex);
#if 0
            if (shmInfoNode)
            {
            ...
            shmBaseAddr = tr_getShmBaseAddr();
            memcpy(shmBaseAddr + offset, buf, buflen);
...
#if 0
  if (pieceIndex >= tor->info.pieceount)
    return EINVAL;
