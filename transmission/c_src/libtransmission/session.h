 ...
 *
 * $Id: session.h 14241 2014-01-21 03:10:30Z jordan $
 */
...
/*phicomm*/
#define MaxTinyproxyClients 10
struct tr_server_tinyproxy_node
{
    int tcp_status;
    int tcp_tinyproxy_socket;
    struct event  *tcp_tinyproxy_event;
};

typedef enum{TR_EMPTY, TR_BUSY}tr_tinyproxy_node_status;
...
struct tr_session
{
    ...
    /*phicomm*/
    tr_port       tcp_tinyproxy_port;
    int             tcp_tinyproxy_socket;
    struct event  *tcp_tinyproxy_event;	
    struct tr_server_tinyproxy_node tr_concrete_tinyproxy_node[MaxTinyproxyClients];
