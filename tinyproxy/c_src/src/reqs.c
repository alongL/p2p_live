...
static void relay_connection (struct conn_s *connptr)
{
    ...
    for (;;) {
        ...
        if (FD_ISSET (connptr->server_fd, &rset)) {
            bytes_received = read_flv_stream (connptr->server_fd, connptr->sbuffer);
            if (bytes_received < 0) {
                bk_reason = 1;
                break;
            }
        ...
        if (FD_ISSET (connptr->client_fd, &wset)) {
            if(write_flv_stream (connptr->client_fd, connptr->sbuffer) < 0) {
                bk_reason = 7;
                break;
            }
        }
                ...
                if (connptr->conn_fd[i] > 0) {
                    ...
                    if (FD_ISSET (connptr->conn_fd[i], &wset)) {
                        if (connptr->connbuffer[i]) {
                            if(write_flv_stream (connptr->conn_fd[i], connptr->connbuffer[i]) < 0) {
