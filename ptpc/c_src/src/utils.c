...
#define DOWNLOAD_MODE_FILE "/var/ptpc/downloadmode"
/* 0: direct 
 * 1: p2p */
int util_getDownloadMode()
{
    int fd,size;
    char buf[256]={0};

    if ((fd=open(DOWNLOAD_MODE_FILE, O_RDONLY)) < 0) {
        log_message (LOG_ERR, "open %s failed!", DOWNLOAD_MODE_FILE);
        return 0;
    }
