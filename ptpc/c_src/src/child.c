...
extern void perform_switch(int v);
extern void print_debug_info(void);
static void child_sigusr_handler(int sig)
{
    if (sig == SIGUSR1) {
        print_debug_info();
    } else if (sig == SIGUSR2) {
...
static void child_main (struct child_s *ptr)
{
    ...
    while (!config.quit) {
        ...
        clilen = addrlen;
        log_message (LOG_INFO, "in child_main() before accept\n");
        connfd = accept (listenfd, cliaddr, &clilen);
        log_message (LOG_INFO, "in child_main() after accept\n");
...
static pid_t child_make (struct child_s *ptr)
{
    ...
    set_signal_handler (SIGUSR1, child_sigusr_handler);
    set_signal_handler (SIGUSR2, child_sigusr_handler);
    ...
    {
        /*cmsLog_error("Failed to make dnsproxy as realtime process");*/
    }
...
short int child_pool_create (void)
{
        ...
        flv_ctx_ptr = (struct flv_s *)malloc_shared_memory (FLV_SHM_SIZE);
        if (!flv_ctx_ptr) {
            log_message (LOG_ERR,
                    "Could not allocate memory for flv stream.");
            return -1;
        }

        memset(flv_ctx_ptr, 0 , sizeof(struct flv_s));
...
void child_main_loop (int fd)
{
    ...
    while (1) {
        ...
        n = select (fd, &rset, NULL, NULL, &timeout);
        ...
        if (n<0) {
            ...
            if (FD_ISSET (fd, &rset)) {
                ...
                if ((conn_fd = accept(fd, (struct sockaddr *)&clientAddr, &sockAddrSize)) < 0)
                {
                    log_message (LOG_ERR, "accept IPC connection failed. errno=%d", errno);
                }
                else
                {
                    log_message (LOG_ERR, "accepted new connection from app on fd %d", conn_fd);
                }
            }
        }
        ...
        if (*servers_waiting < child_config.minspareservers) {
			...
			log_message (LOG_INFO, "child_ptr beror child_make(i=%d)\n", i);
                    child_ptr[i].status = T_WAITING;
                    child_ptr[i].tid =
